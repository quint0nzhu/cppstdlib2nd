/*
  author: Quinton
  date: 2018-11-2
  This code is the source code of The C++ Standary Library 2nd Edition.Chapter 8
*/
...
int main()
{
  ...
  //返回值必在区间[0,bucket_count())内，如果bucket_count()是0，返回值不明确
  ...
  //如果编号为bucketIdx的bucket为空，此调用相当于container::end(bucketIdx)或container::cend(bucketIdx)
  //如果bucket不是区间[0,bucket_count())内的一个有效索引，操作结果不可预测
  ...
  //所有三种形式都返回一个iterator，指向编号为bucketIdx的bucket内的所有元素的终点（最未元素的下一位置）
  //如果编号为bucketIdx的bucket为空，此调用相当于container::begin(bucketIdx)或container::cbegin(bucketIdx)
  ...
  //建立一个新容器，以alloc为内存模型，以initializer-list的元素为初值，始自C++11
  
  //container::container(initializer-list,const CompFunc& cmpPred,const Allocator& alloc)
  //建立一个新容器，拥有排序准则cmpPred，以alloc为内存模型，以initializer-list元素为初值
  //排序准则必须定义出strict weak ordering，始自C++11

  //container::container(initializer-list,size_type bnum,const Hasher& hasher,const KeyEqual& eqPred,const Allocator& alloc)
  //建立一个新空器，带有至少bnum个bucket，以hasher为hashing函数，以eqPred为相等性准则，以alloc为内存模型，以initializer-list为初值
  ...
  //对于第二形式，调用完毕后c仍然有效，但其内容不再明确，始自C++11

  //container::container(size_type num,const T& value,const Allocator& alloc)
  //建立一个新容器，拥有num个元素，使用alloc作为内存模型
  //生成的元素都是value的拷贝，T是容器元素的类型。注意，对string，value以by value的形式传递
  
  //container::container(InputIterator beg,InputIterator end,const Allocator& alloc)
  //建立一个新容器，以区间[beg,end)内的所有元素为初值，使用alloc作为内存模型
  //此函数是个member template。所以只要源区间内的元素能够转换为容器的元素类型，此函数就可执行
  
  //container::container(InputIterator beg,InputIterator end,const CompFunc& cmpPred,const Allocator& alloc)
  //建立一个新容器，拥有排序准则cmpPred，以区间[beg,end)内的所有元素为初值，以alloc为内存模型
  //此函数是个member template。所以只要源区间内的元素能够转换为容器的元素类型，此函数就可执行
  //排序准则必须定义出strict weak ordering
  ...
  //建立一个新容器，带有至少bnum个bucket，以hasher为hashing函数，以eqPred为相等性准则，以alloc为内存模型，以区间[beg,end)内的所有元素为初值
