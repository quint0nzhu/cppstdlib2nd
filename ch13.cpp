...
int main(int argc, char* argv[])
{
 ...
 //这些函数（从索引idx开始）查找“与C-string cstr内容相同”的第一个或最后一个substring
 //函数find()正向(forward)查找，并返回找到的第一个substring
 //函数rfind()反向(backward)搜索，并返回找到的最后一个substring
 //如果成功，就返回substring内的第一字符的索引；否则返回string::npos
 //cstr不可以是null pointer（nullptr或NULL），否则会导致不确定的行为
 ...
 //这些函数（从索引idx开始）查找与“与字符数组chars内的chars_len个字符内容相同”的第一个或最后一个子字符串
 //函数find()正向(forward)查找，并返回找到的第一个substring
 //函数rfind()反向(backward)搜索，并返回找到的最后一个substring
 //如果成功，就返回substring内的第一个字符的索引；否则返回string::npos
 //chars必须至少包含chars_len个字符。这些字符可以为任意值，'\0'无特殊意义
 ...
 //这些函数（从索引idx处开始）查找*this之中属于（或不属于）str的第一个字符
 ...
 //这些函数（从索引idx处开始）查找*this之中属于（或不属于）C-string cstr的第一个字符
 ...
 //这些函数从索引idx处开始查找*this之中“属于（或不属于）字符数组chars内的前chars_len个字符”的第一个字符
 ...
 //这些函数（从索引idx处开始）查找*this之中等于（或不等于）字符c的第一个字符
 //如果成功，就返回找到的字符的索引；否则返回string::npos
 ...
 //这些函数（从索引idx处开始）查找*this之中属于（或不属于）str的最后一个字符
 //如果成功就返回“找到的substring或字符”的索引；否则返回string::npos
 ...
 //这些函数（从索引idx处开始）查找*this之中属于（或不属于）C-string cstr的最后一个字符
 //如果成功就返回字符索引；否则返回string::npos
 //cstr不可以是null pointer（nullptr或NULL），否则会导致不确定的行为
 ...
 //这些函数从索引idx处开始查找*this之中“属于（或不属于）字符数组chars内的前chars_len个字符”的最后一个字符
 //如果成功，就返回找到的字符的索引；否则返回string::npos
 //chars必须至少包含chars_len个字符。这些字符可以为任意值，'\0'无特殊意义
 ...
 //这些函数（从索引idx处开始）查找*this之中等于（或不等于）字符c的最后一个字符
 //如果成功，就返回找到的字符的索引；否则返回string::npos
 ...
 //这几种形式都返回*this之中“从索引idx开始的最多len个字符”所组成的substring
 //如果滑len，则将“余下的所有字符”当作substring返回
 ...
 //如果strm.width()大于0，则至少写入width()个字符，然后width()被设为0
 //ostream的类型是basic_ostream<char>，具体取决于字符类型
 ...
 //-strm.good()为false（这也可能导致相应异常）
 //-对下一个字符c，isspace(c,strm.getloc())为true
 //-已存入str.max_size()个字符
 //视情况重新分配内存
 //istream的类型是basic_istream<char>，具体取决于字符类型
 //在C++11之前，stream的类型是个lvalue reference
 ...
 //所有形式都返回一个reverse random-access iterator，指向倒数第一字符（亦即最末字符）
